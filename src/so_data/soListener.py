...
class GradientSensor(SimpleTopicSensor):
    """
    Gradient Sensor - sensor to receive and store gradient data
    """
    def __init__(self, name, pose_sensor, topic='soData', message_type=soMessage, initial_value=None, create_log=False):
        ...
        self.buffer = soBuffer.SoBuffer(2.0, pose_sensor.topic_name)
    ...
    def update(self, newValue):
        '''
        This method is to store the value in the buffer
        '''
    ...
    def get_gradient_distance(self, pose):
        ...
        self._gradpos = self.buffer.get_current_gradient()
        if self._gradpos:
            distance = [self._gradpos.direction * (self._gradpos.p.x - pose.x),
                        self._gradpos.direction * (self._gradpos.p.y - pose.y)]
            return distance
        else:
            return [0.0, 0.0]
